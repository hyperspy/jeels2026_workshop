...
def _(hs):
    ...
    s = hs.load("datasets/CuZn_EELS_mapping_tutorial.hspy")
    return s,
...
def _(s):
    ...
    s_background_removed = s.remove_background(signal_range=(700., 900.), fast=False)
...
def _(hs):
    ...
    roi_Cu = hs.roi.SpanROI(930, 1005)
    roi_Zn = hs.roi.SpanROI(1045, 1145)
    return roi_Cu, roi_Zn


@app.cell
def _(hs, roi_Cu, roi_Zn, s_background_removed):
    # use `plot_roi_map` and pass the two predefined rois
    hs.plot.plot_roi_map(s_background_removed, rois=[roi_Cu, roi_Zn])
    return
...
@app.cell
def _(s):
    # Display the metadata to see if these metadata are already present in the signal
    s.metadata
    return
...
def _(Cu_map, Zn_map, hs):
    ...
    hs.plot.plot_images([Cu_map, Zn_map])
    return
...
def _(Cu_map, Zn_map, hs):
    ...
    hs.plot.plot_images([Cu_map, Zn_map], overlay=True, label=["Cu", "Zn"], axes_decor="off")
    return
...
def _(Cu_map, Zn_map):
    ...
    Zn_map.save("Zn_map.jpg", scalebar=True, output_size=500)
    return
